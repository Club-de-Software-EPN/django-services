...
class Monster:
    ...
    def __init__(self, initial_position_x, initial_position_y):
        ...
        self.velocity = 16
        self.image_count = 0
        self.image_actual = Monster.images["idle_down"]
        self.image = Monster.images["idle_down"]
        self.direction = -1
        self.tick_count = 1
        self.last_action = -1
    ...
    def action(self, action: int):
        ...
        if self.direction == Monster.DIRECTION_UP:
            # in pygame negative y is UP
            self.position.y -= move_pixels
            self.image_actual = Monster.images['walk_up']
            self.image = Monster.images['walk_up']
        elif self.direction == Monster.DIRECTION_RIGHT:
            self.position.x += move_pixels
            self.image_actual = Monster.images['walk_right']
            self.image = Monster.images['walk_right']
        elif self.direction == Monster.DIRECTION_DOWN:
            self.position.y += move_pixels
            self.image_actual = Monster.images['walk_down']
            self.image = Monster.images['walk_down']
        elif self.direction == Monster.DIRECTION_LEFT:
            self.position.x -= move_pixels
            self.image_actual = Monster.images['walk_left']
            self.image = Monster.images['walk_left']


        THRESHOLD = 100

        if self.position.x < THRESHOLD:
            self.position.x += move_pixels * 2
        elif self.position.x > Parameters.WINDOW_WIDTH - THRESHOLD:
            self.position.x -= move_pixels * 2

        if self.position.y < THRESHOLD:
            self.position.y += move_pixels * 2
        elif self.position.y > Parameters.WINDOW_HEIGHT - THRESHOLD:
            self.position.y -= move_pixels * 2
    ...
    def is_looping(self):
        ...
        if self.actual_action == Monster.DIRECTION_UP and self.last_action == Monster.DIRECTION_DOWN:
            return True
        elif self.actual_action == Monster.DIRECTION_DOWN and self.last_action == Monster.DIRECTION_UP:
            return True
        elif self.actual_action == Monster.DIRECTION_RIGHT and self.last_action == Monster.DIRECTION_LEFT:
            return True
        elif self.actual_action == Monster.DIRECTION_LEFT and self.last_action == Monster.DIRECTION_RIGHT:
            return True
